...
> 封面图：images/ai-daily-2026-07-09.jpg
...
他们做了件很实用的事：让 transformers 的模型在 vLLM 里的推理速度追上（甚至超过）原生 vLLM 实现。原理是用 torch.fx 做静态图分析，找到可以融合的操作，然后用 ast 重写代码，把匹配的算子映射到 vLLM 的优化 kernel 上。实测 Qwen3-4B、32B 和 235B MoE 三个模型，throughput 全部打平或超越原生实现。
...
- "Your coding agent doesn't always follow your rules" - Talha Sheikh（Checkout.com）探讨了 agent 对规则指令的遵循度问题
- "Teaching Coding Agents to do Spreadsheets" - Nuno Campos（Witan Labs）展示了让 coding agent 理解电子表格任务的挑战
- "I Run a Fleet of AI Agents Across Three Machines. Here's What Broke." - Kyle Jaejun Lee（KRAFTON）讲了三机集群上运行 agent 舰队遇到的实操坑
- "Build AI Systems for Discernment, Not Approval" - Angel Ortmann Lee（Duolingo）从 Duolingo 的视角讨论了 agent 系统应该追求判断力而不是审批感
...
原文链接：https://vercel.com/blog/vercel-agent
