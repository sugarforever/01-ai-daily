...
Google I/O 2026 上发布了 Gemini 3.5 系列的首个模型——3.5 Flash。Google 的定位很清晰：「前沿智能 + 行动能力」。3.5 Flash 在 Agent 和代码能力上超越了此前的 Gemini 3.1 Pro，在 Terminal-Bench 2.1（76.2%）、GDPval-AA（1656 Elo）和 MCP Atlas（83.6%）上均取得领先。多模态理解方面 CharXiv Reasoning 得分 84.2%。输出速度方面，比其它前沿模型快 4 倍。

更重要的是它与 Antigravity 的配合。3.5 Flash 配合更新后的 Antigravity harness，可以部署协作子代理来处理大规模复杂任务——一个真实案例是用两个代理协作，在 6 小时内完成了从合成 AlphaZero 论文到编码完整可玩游戏的全流程。
...
- **Google Antigravity 2.0**——Google 新一代 Agent 开发平台的完整介绍。链接：[Introducing Google Antigravity 2.0](https://deepmind.google/blog/introducing-google-antigravity-2/)
